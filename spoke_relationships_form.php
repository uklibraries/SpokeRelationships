...
foreach ($itemtypes as $type) {
    ...
    $idOf[$type['name']] = $type['id'];
}

$item_type = $nameOf[$item->item_type_id];
$parent_id = -1;
switch ($item_type) {
case 'collections':
    $parent_id = $idOf['collections'];
    break;
case 'series':
    $parent_id = $idOf['collections'];
    break;
case 'interviews':
    $parent_id = $idOf['series'];
    break;
}
...
<div id="item-relation-selector" style="overflow: auto; padding: 20px; border-radius: 6px; background: #fff" class="lity-hide">
    ...
    <input type="hidden" id="new_relation_property_id" name="new_relation_property_id" value="7">
    <input type="hidden" id="new_relation_object_item_type_id" name="new_relation_object_item_type_id" value="<?php echo $parent_id; ?>">

    <p>
    <?php echo __('Item Sort'); ?>:
    <fieldset>
        <input type="radio" name="itemsListSort" id="new_selectObjectSortTimestamp" value="timestamp">
        <label for="selectObjectSortTimeStamp">Most recently updated</label>

        <input type="radio" name="itemsListSort" id="new_selectObjectSortName" value="name" checked>
